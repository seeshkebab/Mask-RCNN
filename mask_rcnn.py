...
def calculate_gt_masks(gt_boxes, image_size: Tuple[int, int]):

    B,N,K = gt_boxes.shape
    H, W = image_size
    gt_masks = torch.zeros((B, N, H, W), dtype=torch.float32)

    for j in range(B):
      for i in range(N):
          # print('gt',gt_boxes[i])
          x0, y0, x1, y1 = gt_boxes[j,i]
          x0 = max(0, min(W - 1, int(x0)))
          y0 = max(0, min(H - 1, int(y0)))
          x1 = max(0, min(W - 1, int(x1)))
          y1 = max(0, min(H - 1, int(y1)))
          gt_masks[j, i, y0:y1 + 1, x0:x1 + 1] = 1
...
class MaskRCNN(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = backbone
        self.rpn = rpn
        self.fastrcnn = faster_rcnn
        self.num_classes = num_classes
        self.roi_size = roi_size
        self.batch_size_per_image = batch_size_per_image
        self.fg_iou_thresh=0.5
        self.box_num_samps=512
        self.box_positive=0.25 #box_positive_fraction
        self.box_reg_w=(10,10,5,5)
        self.box_score_thresh=0.1
        self.box_nms_thresh=0.6
        self.box_num_detections=100
        ...
        temp=self.backbone.out_channels
        for n in layers:
            conv=torch.nn.Conv2d(temp,n,kernel_size=3,stride=1,padding=1)
            nn.init.normal_(conv.weight,0,0.01)
            nn.init.constant_(conv.bias,0)
            mask_pred.append(conv)
            mask_pred.append(torch.nn.ReLU())
            temp=n

        conv=torch.nn.ConvTranspose2d(temp,hid_dim,kernel_size=2,stride=2,padding=0)
        nn.init.normal_(conv.weight,0,0.01)
        nn.init.constant_(conv.bias,0)
        mask_pred.append(conv)
        mask_pred.append(torch.nn.ReLU())
        conv=torch.nn.Conv2d(hid_dim,num_classes,kernel_size=1,stride=1,padding=0)
        nn.init.normal_(conv.weight,0,0.01)
        nn.init.constant_(conv.bias,0)
        mask_pred.append(conv)
       
        self.mask_pred = nn.Sequential(*mask_pred)
    ...
    def forward(
        ...
        # print('proposals_per_fpn_level',proposals_per_fpn_level.shape)

        # Get batch size from FPN feats:
        num_images = feats_per_fpn_level["p3"].shape[0]
        # print(feats_per_fpn_level.shape)
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            level_props = proposals_per_fpn_level[level_name]
            level_stride = self.backbone.fpn_strides[level_name]
            # print('props',level_feats.shape)

            roi_feats=torchvision.ops.roi_align(level_feats,level_props,self.roi_size, spatial_scale= 1.0/level_stride, aligned=True)
            #roi_masks
            #need gt masks and level proeps as inputs to it
            # print('gt_masks',gt_mask.device)

            # roi_masks = torchvision.ops.roi_align(gt_mask,level_props,self.roi_size, spatial_scale= 1.0/level_stride, aligned=True)


            roi_feats_per_fpn_level[level_name] = roi_feats
            # roi_masks_per_fpn_level[level_name] = roi_masks

        roi_feats = self._cat_across_fpn_levels(roi_feats_per_fpn_level, dim=0)
        # roi_masks = self._cat_across_fpn_levels(roi_masks_per_fpn_level, dim=0)
        # print(roi_masks.shape)
        # print(roi_feats.shape)
        
        pred_cls_logits = self.mask_pred(roi_feats)
        mask_logit = pred_cls_logits
        # mask_logits = self.mask_pred(roi_masks)
        ...
        sample_pred_cls_logits = pred_cls_logits[indices].to(gt_boxes.device)
        sample_mask_logit = mask_logit[indices].to(gt_boxes.device)
        # print('2',sample_pred_cls_logits.device)
        ...
        ##########################################################################################
        img_h = feats_per_fpn_level["p3"].shape[2] * 8
        img_w = feats_per_fpn_level["p3"].shape[3] * 8
        # print('gt_boxes',level_props.deivce)
        # print('gt_box',gt_boxes.shape)
        gt_label = gt_boxes[:,:,4:]
        # print(gt_label.shape)
        proposal = output_dict["proposals"]
        print('proposals_per_image',proposal.shape)

        gt_mask  = calculate_gt_masks(gt_boxes[:,:,:4], image_size = (img_h,img_w))
        print('gt_mask',gt_mask.shape)
        print('mask_logit',mask_logit.shape)
        gt_mask = gt_mask.to(device='cuda')
        
        # regression_target = gt_box_deltas
        
        

        matched_idx = matched_idx[:, None].to(proposal)
        roi = torch.cat((matched_idx, proposal), dim=1) #match_idx_
        ...
        idx = torch.arange(label.shape[0], device=label.device)
        mask_loss = F.binary_cross_entropy_with_logits(sample_mask_logit, mask_target)
